 ...
 * @author Talha
 */
public class FooseBallController implements Initializable {

    @FXML
    private AnchorPane anc_gametable5;
    @FXML
    private Button b3_add_gametable5;
    @FXML
    private Button b4_cancel_gametable5;
    @FXML
    private JFXTextField tf1_P1name_gametable5;
    @FXML
    private JFXTextField tf2_P2name_gametable5;
    @FXML
    private JFXTextField tf4_starttime_gametable5;
    @FXML
    private JFXTextField tf5_endtime_gametable5;
    @FXML
    private Button b1_start_gametable5;
    @FXML
    private Button b2_end_gametable5;
    @FXML
    private JFXTextField tf6_bill_gametable5;
    @FXML
    private JFXTextField tf3_quant_gametable5;
    ...
    @FXML
    private ChoiceBox<String> ch1_gamelist_gamestable5;
    @FXML
    private JFXTextField tf7_amountpaid_gametable5;
    /**
    ...
    public void initialize(URL url, ResourceBundle rb) {
        // TODO
          ...
          ObservableList<String> dbOrderStatus = FXCollections.observableArrayList("Foose ball");
          ch1_gamelist_gamestable5.setItems(dbOrderStatus);
    }   

    @FXML
    private void f_b3_add_gametable5(ActionEvent event) {
          //It's time to add 
        //Bill is calculated base on number of games
     
        nextgame=ch1_gamelist_gamestable5.getSelectionModel().getSelectedItem();
        if(nextgame.equals("Foose ball")){
        i=i+1;
        bill=bill+50;
        //Only adding here when repetition occurs
        gamelist.add(ch1_gamelist_gamestable5.getSelectionModel().getSelectedItem());
        quantitylist.add(tf3_quant_gametable5.getText());
        tf3_quant_gametable5.setText(String.valueOf(i));
        tf6_bill_gametable5.setText(String.valueOf(bill));
    ...
    @FXML
    private void f_b4_cancel_gametable5(ActionEvent event) {
        ...
        if (reply == JOptionPane.YES_OPTION) {
                  Stage s=(Stage) anc_gametable5.getScene().getWindow();
                  s.close();
        }
        else {
    ...
    @FXML
    private void f_b1_start_gametable5(ActionEvent event) {
        ...
        tf4_starttime_gametable5.setText(sttarttime);
        //Storing name of players in variables
        p1n=tf1_P1name_gametable5.getText();
        p2n=tf2_P2name_gametable5.getText();
    }

    @FXML
    private void f_b2_end_gametable5(ActionEvent event) {
if(tf7_amountpaid_gametable5.getText().isEmpty()){
        ...
        tf5_endtime_gametable5.setText(totalendtime);
        ...
        amountpaid=Float.parseFloat(tf7_amountpaid_gametable5.getText());
        PrinterJob pj = PrinterJob.getPrinterJob();
                
        pj.setPrintable(new K().new L(),new K().getPageFormat(pj));
        try {
             pj.print();
        ...
        //Beforing clearing now we have to send data to database
         ...
         try{
            ...
            psmt.setString(1, "0006");//getting password
            psmt.setString(2, tf2_P2name_gametable5.getText()); 
            psmt.setString(3, tf1_P1name_gametable5.getText());
            psmt.setString(4, tf4_starttime_gametable5.getText()); 
            psmt.setString(5, tf5_endtime_gametable5.getText());
            ...
            psmt.setString(7,tf6_bill_gametable5.getText());//phone no
            ...
            psmt1.setString(4,"Games were played on table 0006 by "+p1n+" and "+p2n);
...
class K extends javax.swing.JFrame{
   //Only extending JFRAME
      FooseBallController  gmp=new FooseBallController();
...
protected static double toPPI(double inch)
    ...
    class L implements Printable{
      long millis=System.currentTimeMillis();  
            java.sql.Date date=new java.sql.Date(millis);
        @Override
        public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {

           int r=FooseBallController.gamelist.size();
        ...
        try{
           ...
           // int headerRectHeighta=40;
            ...
            g2d.drawString("-------------------------------------",12,y);y+=headerRectHeight;
             g2d.drawString("Player 1: "+FooseBallController.p1n+" Player 2: "+FooseBallController.p2n+"",12,y);y+=yShift;
            g2d.drawString("-------------------------------------",10,y);y+=headerRectHeight;
            g2d.drawString(" Games                   ",10,y);y+=yShift;
            g2d.drawString("-------------------------------------",10,y);y+=headerRectHeight;
          
           for(int i=0;i<r;i++){
              g2d.drawString(""+FooseBallController.gamelist.get(i)+" "+"                            ",10,y);y+=yShift;
            g2d.drawString("                          ",10,y); g2d.drawString("",160,y);y+=yShift;

           }   
            g2d.drawString("-------------------------------------",10,y);y+=yShift;
            g2d.drawString(" Total amount:               "+String.valueOf(FooseBallController.bill)+"   ",10,y);y+=yShift;
            g2d.drawString("-------------------------------------",10,y);y+=yShift;
            g2d.drawString(" Cash      :                 "+String.valueOf(FooseBallController.amountpaid)+"   ",10,y);y+=yShift;
            g2d.drawString("-------------------------------------",10,y);y+=yShift;
            g2d.drawString(" Cashback   :                 "+String.valueOf(FooseBallController.amountpaid-FooseBallController.bill)+"   ",10,y);y+=yShift;
