 ...
 * @author Talha
 */
public class GamesTable2 implements Initializable {

    @FXML
    private AnchorPane anc_gametable2;
    @FXML
    private Button b3_add_GameTable2;
    @FXML
    private ChoiceBox<String> ch1_gametype_GameTable2;
    @FXML
    private Button b4_cancel_GameTable2;
    @FXML
    private JFXTextField tf1_P1Name_GameTable2;
    @FXML
    private JFXTextField tf2_P2Name_GameTable2;
    @FXML
    private JFXTextField tf4_starttimee_GameTable2;
    @FXML
    private JFXTextField tf5_endtime_GameTable2;
    @FXML
    private Button b1_start_GameTable2;
    @FXML
    private Button b2_End_GameTable2;
    @FXML
    private JFXTextField tf6_bill_GameTable2;
    @FXML
    private JFXTextField tf3_quantity_GameTable2;
   ...
   public static String p1n,p2n;
    ...
    static ArrayList <String> gamelist2=new ArrayList<>();
    static ArrayList <String> quantitylist=new ArrayList<>();
    public double bHeight=0.0;
    @FXML
    private JFXTextField tf7_bill_GameTable21;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        // TODO
          ...
          ch1_gametype_GameTable2.setItems(dbOrderStatus);
    }    

    @FXML
    private void f_b3_add_GameTable2(ActionEvent event) {
        //It's time to add 
        //Bill is calculated base on number of games
     
        nextgame=ch1_gametype_GameTable2.getSelectionModel().getSelectedItem();
        if(nextgame.equals(prevgame)){
        i=i+1;
        bill=bill+(prevgamebill);
        //Only adding here when repetition occurs
        gamelist2.add(ch1_gametype_GameTable2.getSelectionModel().getSelectedItem());
        quantitylist.add(tf3_quantity_GameTable2.getText());
        tf3_quantity_GameTable2.setText(String.valueOf(i));
        tf6_bill_GameTable2.setText(String.valueOf(bill));
    ...
    @FXML
    private void f_b4_cancel_GameTable2(ActionEvent event) {
        ...
        if (reply == JOptionPane.YES_OPTION) {
                  Stage s=(Stage) anc_gametable2.getScene().getWindow();
                  s.close();
        }
        else {
    ...
    @FXML
    private void F_b1_start_GameTable2(ActionEvent event) {
        ...
        tf4_starttimee_GameTable2.setText(sttarttime);
        //Storing name of players in variables
        p1n=tf1_P1Name_GameTable2.getText();
        p2n=tf2_P2Name_GameTable2.getText();
    }

    @FXML
    private void f_b2_End_GameTable2(ActionEvent event) {
        if(tf7_bill_GameTable21.getText().isEmpty()){
        ...
        tf5_endtime_GameTable2.setText(totalendtime);
        
        //Now it's time to do printing
        //Creating print data what kind od data do you want to print
        //Geetting image;
        
         bHeight = Double.valueOf(gamelist2.size());
        //JOptionPane.showMessageDialog(rootPane, bHeight);
        amountpaid=Float.parseFloat(tf7_bill_GameTable21.getText());
        PrinterJob pj = PrinterJob.getPrinterJob();
                
        pj.setPrintable(new C().new D(),new C().getPageFormat(pj));
        try {
             pj.print();
        ...
        //Beforing clearing now we have to send data to database
         ...
         try{
            ...
            psmt.setString(1, "0002");//getting password
            psmt.setString(2, tf2_P2Name_GameTable2.getText()); 
            psmt.setString(3, tf1_P1Name_GameTable2.getText());
            psmt.setString(4, tf4_starttimee_GameTable2.getText()); 
            psmt.setString(5, tf5_endtime_GameTable2.getText());
            //For adding game types which players have played using arraylist which we have generated                                                 
            for(int count=0;count<gamelist2.size();count++){
            listDB=listDB+gamelist2.get(count)+",";
         }                              
             psmt.setString(6,listDB); //Setting games
            psmt.setString(7,tf6_bill_GameTable2.getText());//phone no
            ...
            psmt1.setString(4,"Games were played on table 0002 by "+p1n+" and "+p2n);
...
class C extends javax.swing.JFrame{
   //Only extending JFRAME
      GamesTable2  gmp=new GamesTable2();
    ...
    class D implements Printable{
      long millis=System.currentTimeMillis();  
        ...
        public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
           ...
           int r=GamesTable2.gamelist2.size();
        ...
        try{
           ...
           // int headerRectHeighta=40;
            ...
            g2d.drawString("-------------------------------------",12,y);y+=headerRectHeight;
             g2d.drawString("Player 1: "+GamesTable2.p1n+" Player 2: "+GamesTable2.p2n+"",12,y);y+=yShift;
            g2d.drawString("-------------------------------------",10,y);y+=headerRectHeight;
            g2d.drawString(" Games                   ",10,y);y+=yShift;
            g2d.drawString("-------------------------------------",10,y);y+=headerRectHeight;
          
           for(int i=0;i<r;i++){
              g2d.drawString(""+GamesTable2.gamelist2.get(i)+" "+"                            ",10,y);y+=yShift;
            g2d.drawString("                          ",10,y); g2d.drawString("",160,y);y+=yShift;

           }   
            g2d.drawString("-------------------------------------",10,y);y+=yShift;
            g2d.drawString(" Total amount:               "+String.valueOf(GamesTable2.bill)+"   ",10,y);y+=yShift;
            g2d.drawString("-------------------------------------",10,y);y+=yShift;
            g2d.drawString(" Cash      :                 "+String.valueOf(GamesTable2.amountpaid)+"   ",10,y);y+=yShift;
            g2d.drawString("-------------------------------------",10,y);y+=yShift;
            g2d.drawString(" Cashback   :                 "+String.valueOf(GamesTable2.amountpaid-GamesTable2.bill)+"   ",10,y);y+=yShift;
