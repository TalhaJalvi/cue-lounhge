 ...
 * @author Talha
 */
public class GamesTable3 implements Initializable {

    @FXML
    private AnchorPane anc_gametable3;
    @FXML
    private Button b3_add_gametable3;
    @FXML
    private ChoiceBox<String> ch1_gametyp_gametable3;
    @FXML
    private Button b4_cancel_gametable3;
    @FXML
    private JFXTextField tf1_P1name_gametable3;
    @FXML
    private JFXTextField tf2_P2name_gametable3;
    @FXML
    private JFXTextField tf4_starttime_gametable3;
    @FXML
    private JFXTextField tf5_endtime_gametable3;
    @FXML
    private Button b1_start_gametable3;
    @FXML
    private Button b2_end_gametable3;
    @FXML
    private JFXTextField tf6_bill_gametable3;
    @FXML
    private JFXTextField tf3_quant_gametable3;
    @FXML
    private JFXTextField tf7_bill_gametable3;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        // TODO
          ...
          ch1_gametyp_gametable3.setItems(dbOrderStatus);
    }    

    @FXML
    private void f_b3_add_gametable3(ActionEvent event) {
          //It's time to add 
        //Bill is calculated base on number of games
     
        nextgame=ch1_gametyp_gametable3.getSelectionModel().getSelectedItem();
        if(nextgame.equals(prevgame)){
        i=i+1;
        bill=bill+(prevgamebill);
        //Only adding here when repetition occurs
        gamelist.add(ch1_gametyp_gametable3.getSelectionModel().getSelectedItem());
        quantitylist.add(tf3_quant_gametable3.getText());
        tf3_quant_gametable3.setText(String.valueOf(i));
        tf6_bill_gametable3.setText(String.valueOf(bill));
    ...
    @FXML
    private void f_b4_cancel_gametable3(ActionEvent event) {
        ...
        if (reply == JOptionPane.YES_OPTION) {
                  Stage s=(Stage) anc_gametable3.getScene().getWindow();
                  s.close();
        }
        else {
    ...
    @FXML
    private void f_b1_start_gametable3(ActionEvent event) {
        ...
        tf4_starttime_gametable3.setText(sttarttime);
        //Storing name of players in variables
        p1n=tf1_P1name_gametable3.getText();
        p2n=tf2_P2name_gametable3.getText();
    }

    @FXML
    private void f_b2_end_gametable3(ActionEvent event) {
        if(tf7_bill_gametable3.getText().isEmpty()){
        ...
        tf5_endtime_gametable3.setText(totalendtime);
        ...
        amountpaid=Float.parseFloat(tf7_bill_gametable3.getText());
        PrinterJob pj = PrinterJob.getPrinterJob();
                
        pj.setPrintable(new E().new F(),new E().getPageFormat(pj));
        try {
             pj.print();
        ...
        //Beforing clearing now we have to send data to database
         ...
         try{
            ...
            psmt.setString(1, "0003");//getting password
            psmt.setString(2, tf2_P2name_gametable3.getText()); 
            psmt.setString(3, tf1_P1name_gametable3.getText());
            psmt.setString(4, tf4_starttime_gametable3.getText()); 
            psmt.setString(5, tf5_endtime_gametable3.getText());
            ...
            psmt.setString(7,tf6_bill_gametable3.getText());//phone no
            ...
            psmt1.setString(4,"Games were played on table 0003 by "+p1n+" and "+p2n);
...
class E extends javax.swing.JFrame{
   //Only extending JFRAME
      GamesTable3  gmp=new GamesTable3();
    ...
    class F implements Printable{
      long millis=System.currentTimeMillis();  
        ...
        public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
           ...
           int r=GamesTable3.gamelist.size();
        ...
        try{
           ...
           // int headerRectHeighta=40;
            ...
            g2d.drawString("-------------------------------------",12,y);y+=headerRectHeight;
             g2d.drawString("Player 1: "+GamesTable3.p1n+" Player 2: "+GamesTable3.p2n+"",12,y);y+=yShift;
            g2d.drawString("-------------------------------------",10,y);y+=headerRectHeight;
            g2d.drawString(" Games                   ",10,y);y+=yShift;
            g2d.drawString("-------------------------------------",10,y);y+=headerRectHeight;
          
           for(int i=0;i<r;i++){
              g2d.drawString(""+GamesTable3.gamelist.get(i)+" "+"                            ",10,y);y+=yShift;
            g2d.drawString("                          ",10,y); g2d.drawString("",160,y);y+=yShift;

           }   
            g2d.drawString("-------------------------------------",10,y);y+=yShift;
            g2d.drawString(" Total amount:               "+String.valueOf(GamesTable3.bill)+"   ",10,y);y+=yShift;
            g2d.drawString("-------------------------------------",10,y);y+=yShift;
            g2d.drawString(" Cash      :                 "+String.valueOf(GamesTable3.amountpaid)+"   ",10,y);y+=yShift;
            g2d.drawString("-------------------------------------",10,y);y+=yShift;
            g2d.drawString(" Cashback   :                 "+String.valueOf(GamesTable3.amountpaid-GamesTable3.bill)+"   ",10,y);y+=yShift;
